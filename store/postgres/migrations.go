...
// Migrations is the grove migration group for the Weave postgres store.
...
`)
			...
			Down: func(ctx context.Context, exec migrate.Executor) error {
				_, err := exec.Exec(ctx, `DROP TABLE IF EXISTS weave_collections CASCADE;`)
				return err
			},
		},
		&migrate.Migration{
...
`)
			...
			Down: func(ctx context.Context, exec migrate.Executor) error {
				_, err := exec.Exec(ctx, `DROP TABLE IF EXISTS weave_documents CASCADE;`)
				return err
			},
		},
		&migrate.Migration{
...
`)
			...
			Down: func(ctx context.Context, exec migrate.Executor) error {
				_, err := exec.Exec(ctx, `DROP TABLE IF EXISTS weave_chunks CASCADE;`)
				return err
			},
		},
	)
