...
description: Complete walkthrough — create a collection, ingest documents, retrieve context, and build a prompt for an LLM.
---
...
func main() {
    ...
    // Connect to PostgreSQL via grove
    db, err := grove.Open("postgres", os.Getenv("DATABASE_URL"))
    if err != nil {
        log.Fatal(err)
    }
    ...
    // pgvector store (shares the same grove.DB connection)
    vecStore := pgvec.New(db)
